...
The "Claude Cowork onboarding for Magic assistants" site. It is **"The Magic Office"**. Since v5 (27 September 2026) the home screen is a first-person **desk**: a practice client texts your phone, tasks sit on sticky notes, and the laptop opens a **practice Claude** that mirrors the real app, where the work happens. Assistants pick the path matching what they were hired for (General admin is live; Bookkeeping, Lead gen, Operations and Content are next). The v4 rooms are the **training floor** (`#/office`), and the reference guide is "the Shelf". Stack: **Vite + React 19 + TypeScript + Tailwind v4 + shadcn/ui (Base UI, `nova` style) + motion**, built to `dist/`. Two older pages (`claude-design.html`, `certificate.html`) are still plain static files served from `public/`. `CONTENT.md` is the how-to for editing paths, missions, scenarios and quiz content. Product plans with internal data live in git-ignored `docs-internal/` (the v5 plan is `docs-internal/PLAN_V5.md`); **the GitHub repo is public**, so never commit telemetry, AL-note content or real names.
...
- `src/main.tsx` mounts `App` and the shadcn `Toaster`. `src/App.tsx` switches on the hash route (`src/lib/routes.ts`): `#/` home (first run, then the desk), `#/start` change path, `#/launchpad` plan your real Week 1, `#/office` training floor, `#/room/<id>` mission, `#/shelf[/<section>]` reference.
...
- `src/world/`: `Lobby` (avatar pick, status card, map), `Map` (2.5D hotspots over the rendered `lobby-1536.webp`, positioned by each room's `spot` in `world.ts`), `RoomPage`, `Shelf`.
