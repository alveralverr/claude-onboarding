...
The "Claude Cowork onboarding for Magic assistants" site. It is **"The Magic Office"**. Since v5 (27 September 2026) the home screen is a first-person **desk**: a practice client texts your phone, tasks sit on sticky notes, and the laptop opens a **practice Claude** that mirrors the real app, where the work happens. Assistants pick the path matching what they were hired for. Five paths are live, each with a fictional client and three shifts: General admin (Priya), Bookkeeping and finance (Marco), Lead gen and sales (Nadia), Operations and data (Tom), Content and social (Leah). The rest are listed and play General admin meanwhile. The desk is the only home: the window on the plate opens **the office** (`#/office`, a map of setup, the safety check and one-skill drills, every one of which exits back to the desk), the books open "the Shelf" (reference), and progress toward client-ready (set up, first shift, safety check) is derived once in `game.ts` and shown in the top-bar pill and the notebook. Stack: **Vite + React 19 + TypeScript + Tailwind v4 + shadcn/ui (Base UI, `nova` style) + motion**, built to `dist/`. Two older pages (`claude-design.html`, `certificate.html`) are still plain static files served from `public/`. `CONTENT.md` is the how-to for editing paths, missions and quiz content. Product plans with internal data live in git-ignored `docs-internal/` (the v5 plan is `docs-internal/PLAN_V5.md`); **the GitHub repo is public**, so never commit telemetry, AL-note content or real names.
...
- `src/main.tsx` mounts `App` and the shadcn `Toaster`. `src/App.tsx` switches on the hash route (`src/lib/routes.ts`): `#/` home (first run, then the desk), `#/start` change path, `#/launchpad` plan your real Week 1, `#/office` the desk with the office map open, `#/drill/<id>` the desk with an update's drill open, `#/room/<id>` mission (retired `inbox` and `clock` redirect), `#/shelf[/<section>]` reference.
- `src/desk/`: `Home`, `FirstRun`, `Desk` (orchestrator), `Stage` (plate, mapped laptop and phone screens, sticky notes, clock, hotspots), `Laptop` (practice Claude frame, step routing, consequence banner), `Phone`, `Coach`, `Summary`, `Playbook`, `Overlay`, `LaunchpadPage`, `homography.ts`, `plateConfig.ts`.
- `src/sim/`: the practice Claude (`Shell`, `Composer`, `parts`, `theme`, `useMenu`, `composerState`).
- `src/story/`: `types.ts` (paths, shifts, tasks, step kinds, habits), `useRunner.ts` (one task at a time: steps, phone thread, consequences, rewind, hints, scoring), `launchpad.ts` (the exact block the `assistant-onboarding` skill parses), `steps/` (one view per step kind).
- `src/content/paths/`: `index.ts` (all twelve role cards) and one folder per live path (`admin/`, `finance/`, `leadgen/`, `ops/`, `content/`: persona, three shifts, launchpad defaults). Shift 1 of every path ends with a `live` step on `FIRST_TASK_KEY`; later shifts use `live-<letter><n>` keys.
- `src/content/updates/`: one typed file per announcement, loaded by glob (`index.ts`); shown on the desk noticeboard, `#/whats-new`, "New" pins, quests and bonus drills (`#/drill/<id>`). Authored with the repo skill `.claude/skills/office-update/`. `src/content/now.ts` is the single source for current model and plan facts; never hardcode model names in copy. Old section hashes (`#setup`, `#connectors`, ...) are aliased in `LEGACY` there.
- `src/world/`: `Map` (the office: 2.5D hotspots over the rendered `lobby-1536.webp`, positioned by each room's `spot` in `world.ts`, plus the grouped room list; opens inside the desk), `RoomPage`, `Shelf`.
- `src/engine/`: `MissionPlayer` plays a mission one step at a time; `steps/` renders each step kind; `bits.tsx` has `Check`, `Shot`, `MediaView`; `burst.ts` is the confetti.
- `src/content/`: `types.ts` (step kinds), `world.ts` (rooms, badges, levels, desk items, avatars), `quests.ts` (weekly quests), `missions/` (one file per room: setup, the Vault, five drills). `src/story/drills.ts` maps each habit to the drill Andi suggests after a weak shift.
...
- `assets-src/v4/`: ChatGPT source renders (PNG, git-ignored, descriptive names) and the image brief `README.md`. Converted WebP copies live in `public/assets/media`.
